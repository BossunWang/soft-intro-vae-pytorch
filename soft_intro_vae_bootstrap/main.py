...
import torch
import argparse
from train_soft_intro_vae_bootstrap import train_soft_intro_vae
...
if __name__ == "__main__":
    ...
    parser.add_argument("-g", "--gamma_r", type=float,
                        help="coefficient for the reconstruction loss for fake data in the decoder", default=1.0)
    ...
    parser.add_argument("-o", "--freq", type=int, help="epochs between copying weights from decoder to target decoder",
                        default=1)
    ...
    train_soft_intro_vae(dataset=args.dataset, z_dim=args.z_dim, batch_size=args.batch_size, num_workers=0,
                         num_epochs=args.num_epochs, copy_to_target_freq=args.freq,
