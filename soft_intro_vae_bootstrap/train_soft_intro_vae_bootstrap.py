"""
Train Soft-Intro VAE for image datasets, using "bootstrapping"
Author: Tal Daniel
...
class SoftIntroVAE(nn.Module):
    def __init__(self, cdim=3, zdim=512, channels=(64, 128, 256, 512, 512, 512), image_size=256, conditional=False,
        ...
        # create a target decoder - we do not train it
        self.target_decoder = Decoder(cdim, zdim, channels, image_size, conditional=conditional,
                                      conv_input_size=self.encoder.conv_output_size, cond_dim=cond_dim)
        ...
        #
        # # create a target decoder - we do not train it
        # self.target_decoder = Decoder_idgan(cdim, zdim)

    def forward(self, x, o_cond=None, deterministic=False, target=True):
        if self.conditional and o_cond is not None:
            ...
            if target:
                y = self.decode_target(z, y_cond=o_cond)
            else:
    ...
    def decode_target(self, z, y_cond=None):
        if self.conditional and y_cond is not None:
            y = self.target_decoder(z, y_cond=y_cond)
        else:
...
def train_soft_intro_vae(dataset='cifar10', z_dim=128, lr_e=2e-4, lr_d=2e-4, batch_size=128, num_workers=4,
                         start_epoch=0, exit_on_negative_diff=False, copy_to_target_freq=1,
                         ...
                         device=torch.device("cpu"), num_row=8, gamma_r=1.0, with_fid=False, log_dir="logs/"):
    """
    ...
    :param copy_to_target_freq: epochs between copying weights from `decoder` to `target_decoder`
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for iteration, batch in enumerate(train_data_loader):
            ...
            if epoch < num_vae:
                ...
                for param in model.target_decoder.parameters():
                ...
                # decode_target
                rec_rec = model.decode_target(z_rec)
                rec_fake = model.decode_target(z_fake)

                # reconstruction loss for the fake data
                # calculate gradient using rec and fake to backprop to update decoder not target_decoder
                lossD_rec_rec = calc_reconstruction_loss(rec, rec_rec, loss_type=recon_loss_type,
                                                         reduction="mean")
                lossD_fake_rec = calc_reconstruction_loss(fake, rec_fake, loss_type=recon_loss_type,
                                                          reduction="mean")
        ...
        if epoch % copy_to_target_freq == 0:
            # copy weights
            model.target_decoder.load_state_dict(model.decoder.state_dict())
...
if __name__ == '__main__':
    ...
    try:
        train_soft_intro_vae(dataset="monsters128", z_dim=128, batch_size=16, num_workers=0, num_epochs=400,
                             ...
                             pretrained=None, copy_to_target_freq=1,
